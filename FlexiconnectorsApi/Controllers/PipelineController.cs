...
namespace MysqlEfCoreDemo.Controllers
{
    public class PipelineController : ControllerBase
    {
        ...
        public PipelineController(MyDbContext dbContext, IConfiguration configuration)
        {
            ...
            hostingfor = _configuration["HostingFor"];
            ...
            uploadfilepath = System.IO.Path.Combine(Directory.GetCurrentDirectory(), "RawFiles") + _slash;
            clonefilepath = _configuration["clonefilepath"];
            v_filepath = System.IO.Path.Combine(Directory.GetCurrentDirectory(), "ExcelScheduler") + _slash;
            errorlogfilePath = System.IO.Path.Combine(Directory.GetCurrentDirectory(), "Errorlog", "error_log.txt");
            comp_file_path = System.IO.Path.Combine(Directory.GetCurrentDirectory(), "CompletedFiles") + _slash;
    ...
    string databasename, string sourcetable, string tvq_type, string user_code)
                        ...
                        if (updpplfieldmap[i].field_type == "INTEGER" || updpplfieldmap[i].field_type == "NUMERIC")
                        {
                            if (updpplfieldmap[i].field_mapped == "Y")
                            {
                                var pplcon = new PipelineCondition()
                                {
                                    ...
                                    condition_type = "Validation",
                                    condition_name = updpplfieldmap[i].ppl_field_name + " Validation",
                                    condition_text = updpplfieldmap[i].field_type == "NUMERIC" ? "([" + updpplfieldmap[i].ppl_field_name + "] IS Null)"
                                      : "([" + updpplfieldmap[i].ppl_field_name + "] IS Null)",
                ...
                try
                {
                    ...
                    if (shdgid.Count > 0)
                    {
                        ...
                        var bulkLoader = new MySqlBulkLoader(connect)
                        {
                            ...
                            FieldTerminator = csvheader[0].column_separator,
                            LineTerminator = lineterm,
                            FileName = csvfilePath,
                            NumberOfLinesToSkip = csvheader[0].number_oflines_toskip,
                            CharacterSet = "utf8",
                            Local = true,
                            Timeout = 0
                        };
        ...
        public async Task<IActionResult> Updatepplapiheader([FromBody] UpdatepplApiHeaderRequest updatepplapihdr)
        {
            try
            {
                ...
                pplapiheader.api_header = updatepplapihdr.api_header;
        ...
        [HttpPost]
        public async Task<IActionResult> Updatepplapiheader1([FromBody] UpdatepplApiHeaderRequest updatepplapihdr)
        {
            var pplapiheader = await dbContext.con_trn_tpplapiheader.FindAsync(Convert.ToInt32(updatepplapihdr.api_gid));
            try
            {
                //string jsonResponse = System.Text.Json.JsonSerializer.Serialize(updatepplapihdr.json_response);
                try
                {
                    using var doc = JsonDocument.Parse(updatepplapihdr.json_response);
                }
            ...
            try
            {
                ...
                if (resultSetStructure != null && resultSetStructure.Any())
                {
                    foreach (var field in resultSetStructure)
                    {
                        ...
                        // Check if record already exists for pipeline_code + dataset_code + sourcefield_name
                        bool exists = await dbContext.con_trn_tpplsourcefield
                            .AnyAsync(f => f.pipeline_code == pipeline_code
                                        && f.dataset_code == dataset_code
                                        && f.sourcefield_name == fieldName
                                        && f.delete_flag == "N");
                ...
                //            /* DataSet Field Save*/
                //            Datasetdetailmodel Datasetdetailmodel = new Datasetdetailmodel();
                //            Datasetdetailmodel.field_name = fieldName.Trim();
                //            Datasetdetailmodel.datasetCode = dataset_code;
                //            Datasetdetailmodel.field_type = "TEXT";
                //            Datasetdetailmodel.field_length = "255";
                //            Datasetdetailmodel.datasetdetail_id = 0;
        ...
        public async Task<JsonResult> NewSchedulerForApi(NewSchedulerForApi objsched)
        {
            ...
            catch (Exception ex)
            {
                var errorLog = new ErrorLog
                {
                    in_errorlog_pipeline_code = ppl_code,
                    in_errorlog_scheduler_gid = sched_gid,
                    in_errorlog_type = "Catch - Method Name : NewSchedulerForApi",
                    in_errorlog_exception = ex.Message,
                    in_created_by = initiated_by
                };
                Errorlog(errorLog);
